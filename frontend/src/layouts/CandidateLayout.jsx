...
import {
  Activity, Home, Briefcase, FileText, ClipboardList,
  Video, BarChart3, User, LogOut, Bell, ChevronRight,
  Menu, X
} from 'lucide-react';

const navItems = [
  { label: 'Beranda', icon: Home, path: '/candidate/dashboard' },
  { label: 'Lowongan', icon: Briefcase, path: '/candidate/jobs' },
  { label: 'Lamaran', icon: FileText, path: '/candidate/applications' },
  { label: 'Tes', icon: ClipboardList, path: '/candidate/tests' },
  { label: 'Wawancara', icon: Video, path: '/candidate/interviews', hideOnMobile: true },
  { label: 'Gap Skill', icon: BarChart3, path: '/candidate/gap-analysis', hideOnMobile: true },
  { label: 'CV Saya', icon: FileText, path: '/candidate/cv-generator', hideOnMobile: true },
  { label: 'Profil', icon: User, path: '/candidate/profile' },
];
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-white border-r border-slate-100 fixed h-full z-30">
        ...
        <div className="flex items-center gap-2.5 px-6 py-5 border-b border-slate-100">
          ...
          <span className="font-bold text-slate-900 text-lg">Zero Gate</span>
        </div>

        {/* Nav */}
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider px-3 mb-2">Menu Utama</p>
          {navItems.map((item) => {
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
              {user?.name?.charAt(0) || user?.email?.charAt(0)?.toUpperCase() || 'U'}
            </div>
            <div className="min-w-0">
              <p className="text-sm font-semibold text-slate-900 truncate">{user?.name || user?.email}</p>
              <p className="text-xs text-slate-500">Pencari Kerja</p>
            </div>
          </div>
          <button
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 z-30 px-2 py-2 safe-area-pb">
